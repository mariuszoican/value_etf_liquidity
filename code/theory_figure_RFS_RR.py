...
plt.plot(
    lmbda_space,
    [f_phi(lmbdai, betas[0]) for lmbdai in lmbda_space],
    label=r"$\beta=%2.1f$" % (betas[0]),
    c="b",
    ls="-",
    lw=1.5,
)
plt.plot(
    lmbda_space,
    [f_phi(lmbdai, betas[1]) for lmbdai in lmbda_space],
    label=r"$\beta=%2.1f$" % (betas[1]),
    c="r",
    ls="--",
    lw=1.5,
)
...
plt.text(
    eq_stable + 0.15,
    h(eta, sigma, eq_stable, beta) + 0.05,
...
plt.text(
    eq_unstable + 0.15,
    h(eta, sigma, eq_unstable, beta) + 0.05,
    r"\noindent Unstable \\ equilibrium ($\lambda^\dagger$)",
    fontsize=18,
)
...
tshares = np.array(
    [
        model([round(beta_space[k], 1), eta, sigma]).turnover_shares(
            feesH_space[k], feesL_space[k]
        )
        for k in range(len(beta_space))
    ]
...
spreads = np.array(
    [
        model([round(beta_space[k], 1), eta, sigma]).spreads(
            feesH_space[k], feesL_space[k]
        )
        for k in range(len(beta_space))
    ]
...
plt.plot(
    beta_space, sharesH_space, ls="-", c="b", label=r"AUM share of ETF $H$ (\%)", lw=2
)
...
plt.plot(
    ...
    ls="--",
    c="r",
    label=r"\noindent Secondary market volume \\ share for ETF $H$ (\%)",
    lw=2,
)
...
plt.plot(
    beta_space,
    np.array(feesH_space) - np.array(feesL_space),
    ls="-",
    c="r",
...
plt.plot(
    beta_space,
    np.array(spreadL_space) - np.array(spreadH_space),
    ls="--",
    c="b",
