...
crsp_data['price']=np.where(crsp_data['PRC']<0, -crsp_data['PRC'],crsp_data['PRC'])
crsp_data['relative_spread']=10000*2*(crsp_data['ASK']-crsp_data['BID'])/(crsp_data['ASK']+crsp_data['BID'])
crsp_data['relative_spread']=np.where(crsp_data['relative_spread']<=0,np.nan,
                                      np.where(crsp_data['relative_spread']>=10000, np.nan, 
                                               crsp_data['relative_spread']))
crsp_data['as_of_date']=crsp_data['date'].apply(lambda x: dt.datetime.strptime(x,"%Y-%m-%d"))
...
wrds_file=wrds_file.merge(crsp_data[['as_of_date','ticker','relative_spread','volume_usd','price']],
                          on=['as_of_date','ticker'],how='left')

wrds_file['aum_index']=wrds_file.groupby(['index_id','as_of_date'])['aum'].transform(sum)
wrds_file['num_hold_index']=wrds_file.groupby(['index_id','as_of_date'])['num_holdings'].transform(np.mean)
wrds_file['volume_index']=wrds_file.groupby(['index_id','as_of_date'])['volume_usd'].transform(sum)
wrds_file['spread_index']=wrds_file.groupby(['index_id','as_of_date'])['relative_spread'].transform(np.mean)

mean_data=wrds_file.groupby('index_id').mean()[['aum_index','num_hold_index',
...
probit_file_2=probit_file.merge(tax_sensitivity,on='index_id',how='left').merge(transient, on='index_id',how='left')
probit_file_2=probit_file_2.drop(['turnover_perc','numholdings_','aum_index','dvol','spread_bps'],axis=1)
